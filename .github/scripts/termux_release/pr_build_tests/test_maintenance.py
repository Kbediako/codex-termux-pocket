...
class PrBuildTests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.request = {
            "format_version": 1,
            "request_id": "pr-build.1",
            "operation": "dispatch-pr-build",
            "pr_number": 7,
    ...
    def api(self, path, *, method="GET", payload=None):
        if method == "POST":
            self.assertEqual(
                path,
                "repos/owner/repo/actions/workflows/termux-mobile-artifact.yml/dispatches",
            )
            self.assertEqual(
                payload, {"ref": self.branch, "inputs": {"source_ref": self.source}}
            )
    ...
    def test_ambiguous_run_identity_fails_before_write(self):
        for changes in (
            ...
            {"head_branch": "main"},
            {"event": "push"},
            {"display_title": "Termux runtime another-source"},
            {"path": ".github/workflows/termux-release-request.yml"},
            {"id": True},
        ):
    ...
    def test_workflow_tree_change_is_rejected(self):
        for entries in (
            [],
            [{"path": ".github/workflows", "type": "dir", "sha": self.source}],
    ...
    def test_moved_branch_and_main_are_rejected(self):
        ...
        self.ref_sha = self.source
        with (
            patch.object(subject, "command", return_value=self.tree),
            patch.object(
            ...
            pr_build.execute(
                self.request, "owner/repo", self.main, self.api, live, subject.command
            )
        self.assertEqual(self.posts, [])
