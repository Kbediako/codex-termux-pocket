...
readonly BASE_MAIN_SHA="8410d9fafca4add1b44dde9ef8c268e70a22747e"
readonly STAGING_BRANCH="automation/alpha1536-integration"
readonly SETUP_SUBJECT="ci: stage exact alpha.6 integration"
readonly UPSTREAM_TAG="rust-v0.153.0-alpha.6"
readonly UPSTREAM_TAG_OBJECT="22a9fdf5f01d606eff192aacb93bc3107688450f"
readonly UPSTREAM_COMMIT="e8b3253fed5aeef7e914441bc3b73b3b0a718b51"
readonly PACKAGE_VERSION="0.153.0-alpha.6"
readonly PREVIOUS_VERSION="0.153.0-alpha.4"
readonly MERGE_SUBJECT="termux: update to 0.153.0-alpha.6"
readonly EXEC_PLAN=".release-engineering/alpha1536-execplan.md"
readonly EVIDENCE_DIR="${RUNNER_TEMP:?RUNNER_TEMP is required}/alpha1536-integration-evidence"
...
on_error() {
  ...
  echo "::error::alpha.6 integration failed in phase ${phase} at line ${line} with exit ${rc}"
  exit "$rc"
}
trap on_error ERR
...
resolve_reviewed_workspace_version_conflict() {
  ...
  base_version="$(git show :1:codex-rs/Cargo.toml | workspace_version)"
  ours_version="$(git show :2:codex-rs/Cargo.toml | workspace_version)"
  theirs_version="$(git show :3:codex-rs/Cargo.toml | workspace_version)"
  [[ "$base_version" == "0.0.0" ]] || return 1
  [[ "$ours_version" == "$PREVIOUS_VERSION" ]] || return 1
  [[ "$theirs_version" == "$PACKAGE_VERSION" ]] || return 1
...
resolved = text.replace(expected, 'version = "0.153.0-alpha.6"\n', 1)
if any(marker in resolved for marker in ("<<<<<<<", "=======", ">>>>>>>")):
    raise SystemExit("unexpected conflict marker remains after the reviewed resolution")
...
PY
  ...
  printf 'resolution=reviewed-workspace-version-only\n' >>"$EVIDENCE_DIR/upstream.env"
  echo "Resolved the sole reviewed conflict by selecting the official alpha.6 workspace version."
}

update_exec_plan_after_merge() {
  python3 - "$EXEC_PLAN" <<'PY'
from pathlib import Path
import sys
...
needle = "## Decision Log\n"
discovery = (
    "- Observation: the exact alpha.6 merge had one conflict only, in the "
    "`[workspace.package]` version line of `codex-rs/Cargo.toml`; index stage 1 "
    "was `0.0.0`, the fork side was `0.153.0-alpha.4`, and upstream was "
    "`0.153.0-alpha.6`.\n"
    "  Evidence: failed integration run 33707625806 and retained artifact "
    "9875788825 captured the combined diff and all three index stages before "
    "the fail-closed retry encoded the resolution.\n\n"
)
if discovery not in text:
    text = text.replace(needle, discovery + needle, 1)
text = text.replace(
...
git checkout -B alpha1536-work "$remote_staging"
...
append_patch_audit \
  "$SETUP_SUBJECT" \
  tooling \
  "Temporary staging-branch job, ExecPlan, and helper used to integrate the exact official alpha.6 source; removed before selecting the release source."
git reset --soft "$BASE_MAIN_SHA"
git add -A
git commit -m "$SETUP_SUBJECT"
setup_commit="$(git rev-parse HEAD)"
git push \
  "--force-with-lease=refs/heads/${STAGING_BRANCH}:${remote_staging}" \
  origin "HEAD:refs/heads/${STAGING_BRANCH}"
...
printf 'upstream_tag=%s\nupstream_tag_object=%s\nupstream_commit=%s\npackage_version=%s\n' \
  "$UPSTREAM_TAG" "$UPSTREAM_TAG_OBJECT" "$UPSTREAM_COMMIT" "$PACKAGE_VERSION" \
  >"$EVIDENCE_DIR/upstream.env"
...
if git merge --no-ff --no-commit "$UPSTREAM_COMMIT"; then
  ...
  collect_diagnostics
  print_conflict_evidence
  if ! resolve_reviewed_workspace_version_conflict; then
...
append_patch_audit \
  "$MERGE_SUBJECT" \
  runtime-critical \
  "Merges the exact official 0.153.0-alpha.6 source and preserves the maintained Android and Termux runtime patch stack."

phase="refreshing-locked-dependency-graphs"
if ! (cd codex-rs && cargo metadata --locked --format-version=1 >/dev/null); then
  echo "Locked Cargo metadata is stale after the exact alpha.6 merge; refreshing Cargo.lock."
...
git push \
  "--force-with-lease=refs/heads/${STAGING_BRANCH}:${setup_commit}" \
  origin "HEAD:refs/heads/${STAGING_BRANCH}"
...
{
  echo "### Exact upstream alpha.6 integration"
  echo
  ...
  echo "- Reviewed conflict: \`codex-rs/Cargo.toml workspace version only\`"
