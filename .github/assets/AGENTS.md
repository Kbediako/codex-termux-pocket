...
Follow the owner's requested publication workflow; do not insist on a PR when
the owner explicitly requests a direct commit to `main`. Never create a permanent
mockup branch. Do not delegate artwork or repository writes to coding bots unless
the owner explicitly approves that delegation. Build changes from the current
target branch and the supplied original capture, not an abandoned PR's history.
